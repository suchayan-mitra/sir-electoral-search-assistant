...
const STATE_CONFIG = {
  ...
  west_bengal: {
    ...
    virama: "্",
    wordOverrides: {
      suchayan: ["সুচয়ন", "সুচায়ন"],
      chandramauli: ["চন্দ্রমৌলি", "চন্দ্রমৌলী"],
      sudipta: ["সুদীপ্ত", "সুদিপ্ত"],
      ghose: ["ঘোষ"],
      mitra: ["মিত্র"],
    },
  },
...
export function transliterateToStateScript(input, state) {
  ...
  return value
    ...
    .split(" ")
    .map((word) => {
      const override = config.wordOverrides?.[word];
...
function curatedStateScriptVariants(input, state) {
  const config = STATE_CONFIG[state];
  if (!config?.wordOverrides || !isLatin(input)) return [];
  const words = clean(input).toLowerCase().split(" ");
  const primary = words.map(
    (word) => config.wordOverrides[word]?.[0] ?? transliterateToStateScript(word, state),
  );
  const variants = [];
  for (let index = 0; index < words.length; index += 1) {
    const alternatives = config.wordOverrides[words[index]]?.slice(1) ?? [];
...
export function generateVariants(input, state, limit = 6) {
  ...
  if (isLatin(base)) {
    ...
    values.push(...curatedStateScriptVariants(base, state));
